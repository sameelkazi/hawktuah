...
const dbPromise = open({
  filename: '/tmp/data/database.sqlite',
...
async function setup() {
  ...
  const operationsCount = await db.get('SELECT count(*) as count FROM operations');
  if (operationsCount.count === 0) {
  ...
  const movesCount = await db.get('SELECT count(*) as count FROM moves');
  ...
  const warehousesCount = await db.get('SELECT count(*) as count FROM warehouses');
  if (warehousesCount.count === 0) {
